...
#
# Arguments:
#   M -- normalized decision matrix with alternatives in rows, criteria in columns
...
#        specifying whether the criterium is to be maximized or minimized
#
# Returns
#   weighted performance matrix, summary of weighted values for alternatives in raw form and as percentages
mcda_wsm <- function(M, w, minmaxcriteria = 'max', VERBOSE = FALSE) {
  ...
  if (!is.numeric(unlist(M))) stop('Only numeric values in performance matrix expected')
  if (!is.vector(w)) stop('weights should be a vector')
  if (!is.numeric(w)) stop('weights should be a numeric vector')
  if (length(w) != ncrit) stop('number of weights does not correspond to number of criteria being used in preference matrix')
