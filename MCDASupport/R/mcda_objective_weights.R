...
#'
#' Then we compute overall performance of the alternatives
#'  Si (step 3)
#'
...
#'
#' \bold{CILOS - Criterion Impact LOSs}
#'
...
#'
#' Finally we construct weight system matrix (F). We start with matrix P and
...
#'  need to be found.
#'
#' This implementation of the method uses singular variable decomposition (SVD) of
...
#'
#' With m being number of alternatives. We follow with calculation of
#'  preference variation value pj
#'
#' \mjsdeqn{p_j = \sum_{i=1}^m (r_{ij} - v_j)^2}
#'
#' Finally we compute the weights:
#'
...
mcda_objective_weights <- function(
  ...
  CILOS <- function(pm, minmax) {
    ncri <- ncol(pm)
    cri <- colnames(pm)
    min <- apply(pm, 2, min) # 2max for criteria being minimalized
    for (i in which(minmax == "min")) {
      pm[, i] <- min[i] / pm[, i]
    }
    pm_norm <- sweep(pm, 2, colSums(pm), "/") # normalize the pm
    l_crit <- apply(pm_norm, 2, max) # largest criteria
    ...
    impact_loss <- colSums(p) # weight system matrix F
    f <- p
    diag(f) <- -1 * impact_loss
    svd_result <- svd(f)
    zero_singular_values <- which(svd_result$d < 1e-10) # Adjust tolerance as needed
    w <- rep(0, times = ncri)
