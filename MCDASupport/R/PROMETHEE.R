# Function provides basic computations for PROMETHEE I, II and III methods. It computes positive and negative
# preference flows which are used as basis of recomendation formulation.
#
# parameters
...
PROMETHEE <- function(PM, preferenceFunction, w, indifferenceTreshold = NULL, prefferenceThreshold = NULL,
  ...
  if (!(is.vector(preferenceFunction))) stop('preferenceFunction must be a vector')
  for(i in 1:ncri){ #check for consistency of preference function
    if(!(preferenceFunction[i] %in% fTypes)) stop('unsupported type of preference function used only: default, U-shape, V-shape, level, linear, Gaussian supported.')
    if(preferenceFunction[i] %in% indifTypes) indifTypesVector[i] <- TRUE
    if(preferenceFunction[i] %in% prefTypes) prefTypesVector[i] <- TRUE
  }
  if (T %in% indifTypesVector){ # indifference threshold required, check its consistency
    if(is.null(qj)) stop('some choosen preference functions need to have set indifference threshold.')
    if (!is.numeric(qj)) stop('indifferenceTreshold must be a numeric vector')
    if (length(qj) != ncri) stop('number of elements in indifferenceTreshold must be equal to number of criteria (set value to 0 for criteria with preference function that does not require this parameter).')
    for(i in 1:ncri){
      if (indifTypesVector[i] && qj[i] < 0){
        msg <- cat('indifference threshold for criterion ', i, ' set < 0, only non negative values accepted.')
        stop(msg)
      }
    }
  }
  if (T %in% prefTypesVector){ # prefference threshold required, check its consistency
    ...
    if (length(pj) != ncri) stop('number of elements in prefference threshold must be equal to number of criteria (set value to 0 for criteria with preference function that does not require this parameter).')
    for(i in 1:ncri){
  ...
  if('Gaussian' %in% preferenceFunction){ # Gaussian type of prefference function check intermediate threshold for consistency
  ...
  if(!is.numeric(w) || length(w) != ncri) stop('weights not set for all criteria.')
  #if(sum(w) != 1) stop('sum of weights does not equal to 1.')
  ...
  Pj <- preferenceDegree(nalt, ncri, DK, preferenceFunction, qj, pj, sj, alt, cri)
