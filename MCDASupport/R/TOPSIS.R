...
TOPSIS <- function(PM, w, minmax = 'max', VERBOSE = FALSE){
  ...
  if (ncol(PM)!=length(w)) stop("length of criteria weights should be checked")
  ...
  #Step 2. Calculation of the Weighted Normalized Decision Matrix (v) (validated)
  v <- sweep(r, MARGIN=2, w, `*`)
  colnames(v) <- cri
  rownames(v) <- alt
  v <- as.data.frame(v)

  #Step 3. Determination of the Ideal (A_ideal) and Anti-ideal (A_anti) Solutions (validated)
  ...
  #Step 4. Calculation of the Separation Measures (distance from ideal/anti ideal solution) (validated)
  D_ideal <- sqrt(sapply(1:nalt, function(i) sum((v[i,] - A_ideal)^2)))
  D_anti <- sqrt(sapply(1:nalt, function(i) sum((v[i,] - A_anti)^2)))
  names(D_ideal) <- alt
  names(D_anti) <- alt

  #Step 5. Calculation of the Relative Closeness to the Ideal Solution (validated)
  C <- D_anti/(D_anti + D_ideal) #always 0-1, the closer to 1, the better
  names(C) <- alt

  #Step 6. order by C
  orderedC <- sort(C, decreasing = T)
