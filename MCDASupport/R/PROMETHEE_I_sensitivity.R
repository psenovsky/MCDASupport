...
PROMETHEE_I_sensitivity <- function(PM, preferenceFunction, w, minmax = "max",
  ...
  for (i in 1:ncri) { # for all criteria
    ...
    for (j in 1:nhyp) { #iterate over values t_* for the criterium
      ...
      thres[i] <- t_p[j]
      tp <- PROMETHEE_I(PM = PM, preferenceFunction = preferenceFunction,
                        w = w, minmax = minmax,
      ...
      thres[i] <- t_q[j]
      tq <- PROMETHEE_I(PM = PM, preferenceFunction = preferenceFunction,
      ...
      thres[i] <- t_i[j]
      ti <- PROMETHEE_I(PM = PM, preferenceFunction = preferenceFunction,
                        w = w, minmax = minmax,
                        indifferenceTreshold = indifferenceTreshold,
                        prefferenceThreshold = prefferenceThreshold,
