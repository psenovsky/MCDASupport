# Function to run sensitivity tests for Electre III method
#
# parameters
...
#   VERBOSE - in verbose mode the function prints out higher amount of
...
Electre_3_sensitivity <- function(PM, w, P, Q, V, minmaxcriteria = "max",
  ...
  Electre_3_paramCheck(PM = PM, w = w, P = P, Q = Q, V = V, alpha = alpha,
                       beta = beta)
  ncri <- length(P)
  nalt <- nrow(PM)
  cri <- colnames(PM)
