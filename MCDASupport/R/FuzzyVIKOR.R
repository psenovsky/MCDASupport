...
#                  (single matrix)
#   w - weights (matrix of weights - decision makers in columns and criteria in
#       rows)
...
FuzzyVIKOR <- function(PM, dictionaryPM, w, dictionaryW, alt, v = NULL,
                       VERBOSE = FALSE) {
  ...
  criteria <- rownames(w)
  if (ncol(w) != n) stop("number of decission makers in w and PM is not same")
  ncri <- nrow(w)
  if (nrow(PM) %% ncri != 0) {
  ...
  crisp_weights <- defuzz_m(w2)
  names(crisp_weights) <- criteria
  crisp_alternative_ratings <- matrix(defuzz_m(f_rdm_all), nrow = nalt)
