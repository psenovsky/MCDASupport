# Sensitivity analysis for PROMETHEE I method. Sensivity of results is checked
...
PROMETHEE_II_sensitivity <- function(PM, preferenceFunction, w, minmax = "max",
  ...
  for (i in 1:ncri) { # for all criteria
    ...
    for (j in 1:nhyp) { #iterate over values t_* for the criterium
      ...
      thres[i] <- t_p[j]
      tp <- PROMETHEE_II(PM = PM, preferenceFunction = preferenceFunction,
                         w = w, minmax = minmax,
                         indifferenceTreshold = indifferenceTreshold,
                         prefferenceThreshold = thres,
                         intermediateThreshold = intermediateThreshold,
                         VERBOSE = FALSE)
      thres <- indifferenceTreshold
      thres[i] <- t_q[j]
      tq <- PROMETHEE_II(PM = PM, preferenceFunction = preferenceFunction,
                         w = w, minmax = minmax, indifferenceTreshold = thres,
                         prefferenceThreshold = prefferenceThreshold,
                         intermediateThreshold = intermediateThreshold,
                         VERBOSE = FALSE)
      thres <- intermediateThreshold
      thres[i] <- t_i[j]
      ti <- PROMETHEE_II(PM = PM, preferenceFunction = preferenceFunction,
                         w = w, minmax = minmax,
                         indifferenceTreshold = indifferenceTreshold,
                         prefferenceThreshold = prefferenceThreshold,
