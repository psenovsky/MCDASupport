...
#   bw_perf is the array with the best and worst performances, and differences between them
...
VIKORIndexes <- function(car, bw_perf, cw, v = NULL){
  #validate params
  if(ncol(bw_perf) != 3) stop('bw_perf parameter is expected to have 3 columns (best, worst and difference).')
  nalt <- nrow(car)
  ncri <- ncol(car)
  alt <- rownames(car)
  if(nrow(bw_perf) != ncri) stop('number of criteria detected in car and bw_perf differs')
  if(length(cw) != ncri) stop('number of criteria detected in car and cw differs')
  #end of validation
  ...
  orderQ <- sort(Q)
  compromise <- list()
  if(orderQ[2] - orderQ[1] >= 1/(nalt - 1)){ #C1 = T
    nS <- names(orderS)
    nR <- names(orderR)
    nQ <- names(orderQ)
    if((v > 0.5 && nQ[1] == nS[1]) || (v < 0.5 && nQ[1] == nR[1]) || (v == 0.5 && nQ[1] == nR[1] && nQ[1] == nS[1])){
      compromise[1] <- nQ[1]
