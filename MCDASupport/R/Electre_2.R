...
#   w  - weights
#   minmaxcriteria - information on "direction" of the criteria (min/max, max is default)
...
Electre_2 <- function(PM, w,
  ...
  Electre_2_paramCheck(PM, w) # common consistency check (common with Electre_2_sensitivity function)
  ...
  PM <- util_pm_minmax(PM, minmaxcriteria) #validate minmax and invert scales if neccessary
  if(!is.numeric(c_minus) || c_minus < 0 || c_minus > 1) stop('Concordance threshold (minimum) out of bounds')
  if(!is.numeric(c_zero) || c_zero < 0 || c_zero > 1) stop('Concordance threshold (middle) out of bounds')
  if(!is.numeric(c_plus) || c_plus < 0 || c_plus > 1) stop('Concordance threshold (maximum) out of bounds')
  if(!is.numeric(d_minus) || d_minus < 0 || d_minus > 1) stop('Discordance threshold (minimum) out of bounds')
  if(!is.numeric(d_plus) || d_plus < 0 || d_plus > 1) stop('Discordance threshold (maximum) out of bounds')
  ...
  #computes preference ratio P+/P- and constructs strong or weak outranking matrixes
  # in dominance_s or dominance_w
  preference_ratio <- function(dom, PM){
    nalt <- length(rownames(PM))
    ncri <- length(colnames(PM))
  ...
  ranking <- function(dom_s, dom_w, alt){
    #py code: dominance = np.clip(2*dominance_s + dominance_w, 0, 2) #since dom_s and _w should consist of 0/1:
    ...
    while (length(y) > 0){
      ...
      for(j in 1:nalt){
        ...
        if (check_d == 0 && sum(dominance[,j]) != -nalt) d[length(d)+1] <- alt[j]
      }
      idx <- match(d, alt)  #get index of alternatives in D set
      for(m in 1:length(d)){
        for(n in 1:length(d)){
          if (dominance[idx[m], idx[n]] == 1){
            #print(cat("Weak relation between ", idx[m], " and ", idx[n])) #ladění
            #Those options within D that are linked to each other by a weak outranking
            #relationship constitute the set U
      ...
      if(length(idx) > 0){
        for(m in 1:length(u)){
          ...
          if (check_b == 0){
            #The set B consists of all options from U not weakly outranked by any other from within U.
            if(!(alt[idx[m]] %in% b)) b[length(b)+1] <- alt[idx[m]]
          }
        }
      }
      # The single option or group of equally ranked options is defined by the union of the sets (D - U) and B
      a <- d[!(d %in% u)]
      #TODO review function of ELECTRE II ranking - how it processes results next 2 lines look weird
  ...
  dominance_s <- ifelse((cm >= t(cm) & lower.tri(cm) & ((cm >= c_plus & dm <= d_plus) | (cm >= c_zero & dm <= d_minus))), 1, dominance_s)
  dominance_w <- ifelse((cm >= t(cm) & lower.tri(cm) & ((cm >= c_zero & dm <= d_plus) | (cm >= c_minus & dm <= d_minus))), 1, dominance_w)
  diag(dominance_s) <- 0
  diag(dominance_w) <- 0
  ...
  #The second pre-order is obtained using the inverse ranking procedure. The same basic
  #algorithm as above is utilised, with the following modifications.
  #invert the direction of the strong and weak outranking graphs GF, and Gf,
  #the ranks obtained from the inverted graphs using the direct procedure is adjusted as follows:
  #'inverse procedure' rank (r2(a»=1+(the number of ranks) (r'2(a)max) -'direct procedure' ranking (r'2(a))
  ...
  for(i in 1:nalt){
    ...
    for(j in 1:nalt){
      if(i != j){
        ...
        #if a is preferred to b in 2 complete pre-orders, then this will also be the case in final pre-order
        if(r1_a > r2_a && r1_d > r2_d) pref[i,j] <- 1
        #if a has an equivalent ranking to b in the one of the complete pre-orders, but is
        #preferred to b in the other, then a precedes b in the final pre-order
        if((r1_a == r2_a && r1_d > r2_d) || (r1_a > r2_a && r1_d == r2_d)) pref[i,j] <- 1
        #if a is preferred to b in the first complete pre-order, but b is preferred to a in the
        #second, then the two options are incomparable in the final pre-order.
        if((r1_a > r2_a && r1_d < r2_d) || (r1_a < r2_a && r1_d > r2_d)) incompar[i,j] <- 1
      }
    }
  }
  colnames(pref) <- alt
  ...
  # does not provide rank but a degree of preference, which can be directly translated to rank
  rank_F <- rowSums(pref)
  names(rank_F) <- alt
