...
VIKOR <- function(PM, w, minmax = 'max', v = NULL, VERBOSE = FALSE){
  ...
  #Step 1. Determine the Best and the Worst Values of All Criteria Functions (validated)
  #f*j = f_max, f-j = f_min
  f_max <- PM %>%
    group_by() %>%
  ...
  #here original VIKOR stops, other steps implemented in extended version of VIKOR
