...
Electre_1S_sensitivity <- function(PM, w, Q, P, V, minmaxcriteria = 'max', lambda = 0.5){
  ...
  for(i in 1:ncri){ # for all criteria
    ...
    for(j in 1:nhyp){ #iterate over values t_* for the criterium
      thres <- P
      thres[i] <- t_p[j]
      tp <- Electre_1S(PM = PM, w = w, P = thres, Q = Q, V = V, minmaxcriteria, lambda = lambda, FALSE)
      #df <- rbind(df, df_row(tp$Kernel, alt = alt, sensitivity = 'P', criterium = cri[i], val = paste(thres, collapse = ', ')))
      df <- rbind(df, df_row(tp$Kernel, alt = alt, sensitivity = 'P', criterium = cri[i], val = thres[i]))
      thres <- Q
      thres[i] <- t_q[j]
      tq <- Electre_1S(PM = PM, w = w, P = P, Q = thres, V = V, minmaxcriteria, lambda = lambda, FALSE)
      # df <- rbind(df, df_row(tq$Kernel, alt = alt, sensitivity = 'Q', criterium = cri[i], val = paste(thres, collapse = ', ')))
      df <- rbind(df, df_row(tq$Kernel, alt = alt, sensitivity = 'Q', criterium = cri[i], val = thres[i]))
      thres <- V
      thres[i] <- t_v[j]
      tv <- Electre_1S(PM = PM, w = w, P = P, Q = Q, V = thres, minmaxcriteria, lambda = lambda, FALSE)
      # df <- rbind(df, df_row(tv$Kernel, alt = alt, sensitivity = 'V', criterium = cri[i], val = paste(thres, collapse = ', ')))
      df <- rbind(df, df_row(tv$Kernel, alt = alt, sensitivity = 'V', criterium = cri[i], val = thres[i]))
    }
  }
