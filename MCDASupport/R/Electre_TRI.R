...
#   VERBOSE        - should the result be printed to console, FALSE = only return the results (without printing them first)
...
Electre_TRI <- function(PM, profiles, profiles_names,
  ...
  alt  <- rownames(PM)
  cri  <- colnames(PM)
  if (!(is.matrix(profiles) || is.data.frame(profiles))) stop('wrong profiles, should be a matrix or a data frame')
  if (is.null(dim(profiles))) stop('less than 2 criteria or 2 profiles')
  if (!is.vector(profiles_names)) stop('profiles_names should be a vector')
  npr <- length(profiles_names) # number of profiles
  if (npr != nrow(profiles)) stop('Number of stated profile names must correspond to number of rows in profile matrix')
  ...
  cj_inv <- list() # inverse list of partial concordance indexes for profiles: cj(bh,a)
  dj     <- list() # list of discordance indexes: dj(a,bh)
  dj_inv <- list() # inverse list of discordance indexes: dj(bh,a)
  d  <- matrix(data = 0, nrow = nalt, ncol = npr)  # template zero matrix of n x m, n = no. of alternatives, m = number of profiles
  ...
  for(j in 1:ncri){
    ...
    for(i in 1:nalt){
      for(k in 1:npr){
        ...
        cj_ab[i,k] <- ifelse(diff_ab >= P[j], 0, ifelse(diff_ab <= Q[j], 1, (P[j] + PM[i,j] - profiles[k,j])/diff_pj))
        cj_ba[i,k] <- ifelse(diff_ba >= P[j], 0, ifelse(diff_ba <= Q[j], 1, (P[j] + profiles[k,j] - PM[i,j])/diff_pj))
  ...
  for(j in 1:ncri){
    ...
    for(i in 1:nalt){
      for(k in 1:npr){
        if(dj_ab[i,k] > oci[i,k]) cr[i,k] <- cr[i,k] * (1 - dj_ab[i,k])/(1 - oci[i,k])
        if(dj_ba[i,k] > oci_inv[i,k]) cr_inv[i,k] <- cr_inv[i,k] * (1 - dj_ba[i,k])/(1 - oci_inv[i,k])
      }
    }
  }
  ...
  for(i in 1:nalt){
    for(k in 1:npr){
      if(cr[i,k] >= lambda && cr_inv[i,k] >= lambda){
        # cr(a,bh) >= lambda and cr(bh,a) >= lambda => aSbh and bhSa => a is indifferent to bh
        pref[i,k] <- 'I' # aIbh
      }else if(cr[i,k] >= lambda && cr_inv[i,k] < lambda){
        # cr(a,bh) >= lambda and cr(bh,a) < lambda => aSbh and not bhSa => a > bh, i.e. a is preferred to bh (weakly or strongly)
        pref[i,k] <- '>' # aPbh
      }else if(cr[i,k] < lambda && cr_inv[i,k] >= lambda){
        # cr(a,bh) < lambda and cr(bh,a) >= lambda => not aSbh and bhSa => bh > a, i.e. bh is preferred to a (weakly or strongly)
        pref[i,k] <- '<' # bhPa
      }else if(cr[i,k] < lambda && cr_inv[i,k] < lambda){
        # cr(a,bh) < lambda and cr(bh,a) < lambda => not aSbh and not bhSa => aRbh, i.e. a is incomparable to bh
        pref[i,k] <- 'R' # aRbh
      }
    }
  }
  ...
  #TODO předělat
  # pessimistic = matrix(0, nalt, npr+1)
  # rownames(pessimistic) <- alt
  # colnames(pessimistic) <- profiles_names
  ...
  # - Each option must be assigned to one reference category only (uniqueness/unicity).
  # - The assignment of any option to its allotted category is not dependent on the assignment of any of the other options (independence).
  # - The procedure for assigning options to categories must be entirely consistent with the design of the reference options themselves (conformity).
  # - When two options have the same outranking relationship with a given reference option, they must be assigned to the same category (homogeneity).
  # - If option a' outranks a, then a' must be assigned to a category at least as good as the one to which a is assigned (monotonicity).
  # - The grouping together of two neighbouring categories must not cause the alteration of options to categories not affected by the alteration (stability)
