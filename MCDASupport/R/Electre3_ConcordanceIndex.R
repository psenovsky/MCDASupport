# Method for computing Concordance Indexes refactored from ELECTRE III method as it is also utilized by ELECTRE 1S method
# parameters:
...
Electre3_ConcordanceIndex <- function(PM, P, Q, w){
  ...
  for(k in 1:ncri){      #index for criteria
    ...
    for(i in 1:nalt){    #index for a
      ...
      for(j in 1:nalt){  #index for b
        #concordance matrix
        #represents percentage of weights of criteria that concord with the proposition 'a outranks b'
        # C(a,b) = 1/W * SUM(wj*cj(a,b))
        # W = SUM(wj)
        ...
        if(PM[i,k] + Q[k] >= PM[j,k]){
          ...
          c[i,j] <- ifelse((diff_piqk != 0), (PM[i,k] - PM[j,k] + P[k])/diff_pq, 1)
          # if(P[i] - Q[k] != 0){
          #   c[i,j] <- (PM[i,k] - PM[j,k] + P[k])/diff_pq
          # }else{
          #   c[i,j] <- 1
          # }
        }
  ...
  cm <- Reduce('+', cjw)/sum(w) #cm = sum(cjw)/sum(w)
  colnames(cm) <- alt
  rownames(cm) <- alt
  out <- list(
